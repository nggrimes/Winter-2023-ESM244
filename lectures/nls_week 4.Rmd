...
### .blue[Instead we iteratively approximate the solution through algorithms]

- .big[Gauss-Newton (Most Common)]

- .big[Levenberg-Marquardt (More flexible)]

### .blue[In general the algorithms make an approximation of the functions gradient (think derivative), then move along until some convergence criteria is met]
...
.pull-right[
### .blue[Global minimum at a=2.25]

### .blue[Begins at initial guess of a=3.5]

### .blue[Step size depends on the 2nd order approximation]

### .blue[Keeps going until the] .green[green line (first derivative)] .blue[ reaches close to zero]
]
...
### .blue[NLS is only as good as the underlying model. Bring your brain to the party and make sure the model you're fitting is appropriate]

### .blue[Follows gradient of steepest descent] $\rightarrow$ .blue[local min/max valleys]
...
### .blue[Requires good initial guesses]
...
```{r bisongraph, out.width="55%",fig.align='center'}
...
.big[1) Use past parameters from similar studies
]

.big[2) Use data to internally define guesses (min, mean, max, etc.)]

.big[3) In 2-D, look at the graphs and estimate
]

.big[4) In N-D, combine steps 1-2 then create a start grid to search over
]
---
...
<uc-blockquote> Best to use methods that you understand than ones you don't </uc-blockquote>
